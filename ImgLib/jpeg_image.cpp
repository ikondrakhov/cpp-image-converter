...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
    ...
    std::vector<JSAMPLE> img_buffer(row_stride);
    while (cinfo.next_scanline < cinfo.image_height) {
    /* jpeg_write_scanlines expects an array of pointers to scanlines.
     * Here the array is only one element long, but you could pass
     * more than one scanline at a time if that's more convenient.
     */
        const Color* row = image.GetLine(cinfo.next_scanline);
        for(int i = 0; i < image.GetWidth(); i++) {
            img_buffer[i * 3] = static_cast<JSAMPLE>(row[i].r);
            img_buffer[i * 3 + 1] = static_cast<JSAMPLE>(row[i].g);
            img_buffer[i * 3 + 2] = static_cast<JSAMPLE>(row[i].b);
        }
        row_pointer[0] = img_buffer.data();
